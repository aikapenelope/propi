...
// ---------------------------------------------------------------------------
// Agent Profiles (public portal for each agent)
// ---------------------------------------------------------------------------

/** Public-facing agent profile for the mini portal at /agente/[slug].
 *  One profile per Clerk userId. The slug is the URL-friendly identifier. */
export const agentProfiles = pgTable(
  "agent_profiles",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: text("user_id").notNull().unique(),
    /** URL slug: /agente/carlos-mendez */
    slug: varchar("slug", { length: 100 }).notNull().unique(),
    /** Display name (may differ from Clerk name) */
    displayName: varchar("display_name", { length: 255 }).notNull(),
    /** Short bio / tagline */
    bio: text("bio"),
    /** Public phone number (tap-to-call) */
    phone: varchar("phone", { length: 50 }),
    /** Public email */
    email: varchar("email", { length: 255 }),
    /** Profile photo URL (Clerk avatar or custom) */
    photoUrl: text("photo_url"),
    /** Agency / company name */
    agency: varchar("agency", { length: 255 }),
    /** City where the agent operates */
    city: varchar("city", { length: 255 }),
    /** WhatsApp number for direct contact button */
  ...
  (table) => [
    index("agent_profiles_slug_idx").on(table.slug),
    index("agent_profiles_user_idx").on(table.userId),
  ],
);

export const agentProfilesRelations = relations(agentProfiles, () => ({
  // userId is a Clerk ID, not a DB FK.
  // Properties are linked via userId match, not a direct FK.
}));
